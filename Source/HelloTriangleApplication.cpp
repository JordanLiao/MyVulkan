...
/*
*   Notice how renderpass's members do not have a reference to a graphics pipeline, even though
*   renderpass will use a pipeline.
*/
void HelloTriangleApplication::createRenderPass() {
    ...
    colorAttachment.loadOp = VK_ATTACHMENT_LOAD_OP_CLEAR; //clear the FB before rendering
    colorAttachment.storeOp = VK_ATTACHMENT_STORE_OP_STORE; //FB would be read after rendering
    ...
    colorAttachmentRef.attachment = 0; //since we only have one color attachment, we reference the first one at index 0
    ...
    //subpass can also have other attachments like depth and stencil, but here we only want color.

    //dependencies describe the relationship between every two subpasses
    VkSubpassDependency dependency{};
    dependency.srcSubpass = VK_SUBPASS_EXTERNAL;
    dependency.dstSubpass = 0;
    //the external subpass stage that subpass0 is waiting on
    dependency.srcStageMask = VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT | VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT;
    //the specific subpass0 stage that is waiting
